...
export class TodoService {
  ...
  async getAll() {
    ...
    this.todoAuthor = this.todoList.map((p) => {
      let author = users.find((u) => u.id === p.userId);
  ...
  getUserPosts(){
    ...
    let users:iUser[] = this.userSvc.getAll();



    this.usersPosts = users.map((u) => {
      const allTodos = this.todoList.filter(p => p.userId == u.id);
